		...
		if (isPlayer && this.j2.hasFlag(player, Flag.FUN)) {
			if (args.length == 0) {
				player.sendMessage(ChatColor.RED
						+ "Correct usage is: /i [item](:damage) (amount)");
				return;
			}
			...
			if (args.length > 1) {
				...
				try {
					...
					player.sendMessage(ChatColor.RED + countString
							+ " is not a number");
					return;
				}
			}
			if (args.length == 3 && this.j2.hasFlag(playerName, Flag.ADMIN)) {
				...
				if (targetPlayer == null) {
					player.sendMessage(ChatColor.RED + targetName
							+ " is not a player");
					return;
				}
			}
			...
			if (!this.j2.hasFlag(player, Flag.ADMIN)
					&& this.j2.isOnSummonlist(itemMaterial.getId())) {
				player.sendMessage(ChatColor.RED
						+ "Can't give that to you right now");
				return;
			}
			if (itemDamage != null) {
				targetPlayer.getInventory().addItem(
						new ItemStack(itemMaterial, itemCount, (short) 0,
								itemDamage));
			} else {
				targetPlayer.getInventory().addItem(
						new ItemStack(itemMaterial, itemCount));
			}
			player.sendMessage("Given " + targetPlayer.getDisplayName() + " "
					+ itemCount + " " + itemMaterial.toString());
			this.j2.log("Giving " + playerName + " " + itemCount + " "
					+ itemMaterial.toString());
			if ((this.j2.isOnWatchlist(itemMaterial.getId()))
					&& (itemCount > 10 || itemCount < 1)) {
				this.j2.irc.messageAdmins("Detecting summon of " + itemCount
						+ " " + itemMaterial.toString() + " by " + playerName);
				this.j2.sendAdminPlusLog(ChatColor.LIGHT_PURPLE
						+ "Detecting summon of " + ChatColor.WHITE + itemCount
						+ " " + ChatColor.LIGHT_PURPLE
						+ itemMaterial.toString() + " by " + ChatColor.WHITE
						+ playerName);
			}
			return;
		}
